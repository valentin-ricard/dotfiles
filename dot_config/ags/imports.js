...
const { exec } = Utils;
const SCALE = Number(exec(`hyprctl monitors -j | jq '.[] | select(.focused | "true") | .scale'`));
const SCREEN_WIDTH = Number(exec(`bash -c "xrandr --current | grep '*' | uniq | awk '{print $1}' | cut -d 'x' -f1 | head -1" | awk '{print $1}'`) * (1 / 1.5));
const SCREEN_HEIGHT = Number(exec(`bash -c "xrandr --current | grep '*' | uniq | awk '{print $1}' | cut -d 'x' -f2 | head -1" | awk '{print $1}'`) * (1 / 1.5));
